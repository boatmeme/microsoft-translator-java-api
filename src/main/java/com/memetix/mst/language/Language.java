...
public enum Language {
	AUTO_DETECT(""), ARABIC("ar"), BULGARIAN("bg"), CATALAN("ca"), CHINESE_SIMPLIFIED("zh-CHS"), CHINESE_TRADITIONAL(
			"zh-CHT"), CZECH("cs"), DANISH("da"), DUTCH("nl"), ENGLISH("en"), ESTONIAN("et"), FINNISH("fi"), FRENCH(
					"fr"), GERMAN("de"), GREEK("el"), HAITIAN_CREOLE("ht"), HEBREW("he"), HINDI(
							"hi"), HMONG_DAW("mww"), HUNGARIAN("hu"), INDONESIAN("id"), ITALIAN("it"), JAPANESE(
									"ja"), KOREAN("ko"), LATVIAN("lv"), LITHUANIAN("lt"), MALAY("ms"), NORWEGIAN(
											"no"), PERSIAN("fa"), POLISH("pl"), PORTUGUESE("pt"), ROMANIAN(
													"ro"), RUSSIAN("ru"), SLOVAK("sk"), SLOVENIAN("sl"), SPANISH(
															"es"), SWEDISH("sv"), THAI("th"), TURKISH("tr"), UKRAINIAN(
																	"uk"), URDU("ur"), VIETNAMESE("vi");
	 ...
	 * the same, single call anyway.
	 * 
	 * @return The String representation of this language's localized Name.
	 */
	public String getName(Language locale, String clientId, String clientSecret) throws Exception {
		...
		if (this.localizedCache.containsKey(locale)) {
			...
			if (this == Language.AUTO_DETECT || locale == Language.AUTO_DETECT) {
				...
				// locale
LanguageService ls = new LanguageService(clientId, clientSecret);
				String[] names = ls.execute(Language.values(), locale);
				int i = 0;
	...
	public static List<String> getLanguageCodesForTranslation(String clientId, String clientSecret) throws Exception {
		GetLanguagesForTranslateService ls = new GetLanguagesForTranslateService(clientId, clientSecret);
		String[] codes = ls.execute();
	 ...
	 *
	 * @param locale
	 *            The Language we should localize the Language names with
	 * @return A Map of all supported languages stored by their localized name.
	 */
	public static Map<String, Language> values(Language locale, String clientId, String clientSecret) throws Exception {
		Map<String, Language> localizedMap = new TreeMap<String, Language>();
		for (Language lang : Language.values()) {
			if (lang == Language.AUTO_DETECT) {
				localizedMap.put(Language.AUTO_DETECT.name(), lang);
			} else {
				localizedMap.put(lang.getName(locale, clientId, clientSecret), lang);
			}
		}
		return localizedMap;
	}
	...
	private final static class LanguageService extends MicrosoftTranslatorAPI {
		public LanguageService(String clientId, String clientSecret) {
		...
		public String[] execute(final Language[] targets, final Language locale) throws Exception {
			...
			final URL url = new URL(SERVICE_URL + PARAM_LOCALE + URLEncoder.encode(locale.toString(), ENCODING)
	...
	private final static class GetLanguagesForTranslateService extends MicrosoftTranslatorAPI {
		...
		public GetLanguagesForTranslateService(String clientId, String clientSecret) {
			super(clientId, clientSecret);
		}
