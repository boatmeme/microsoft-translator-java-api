...
public abstract class MicrosoftTranslatorAPI {
	...
	protected static final String PARAM_TO_LANG = "&to=", PARAM_FROM_LANG = "&from=", PARAM_TEXT_SINGLE = "&text=",
			PARAM_TEXT_ARRAY = "&texts=", PARAM_SPOKEN_LANGUAGE = "&language=", PARAM_SENTENCES_LANGUAGE = "&language=",
			PARAM_LOCALE = "&locale=", PARAM_LANGUAGE_CODES = "&languageCodes=", PARAM_CATEGORY = "&category=",
	...
	/**
	 * Constructor using the Client and Client ID. All new applications should
	 * obtain a ClientId and Client Secret by following the guide at:
	...
	private String retrieveResponse(final URL url) throws Exception {
		if (clientId != null && clientSecret != null && System.currentTimeMillis() > tokenExpiration) {
			String tokenJson = getToken(referrer, clientId, clientSecret);
	 ...
	 * 
	 * @param referer
	 *            The referrer (can be null)
	 * @param clientId
	 *            The Client key.
	 * @param clientSecret
	 *            The Client Secret
	 */
	public static String getToken(final String referrer, final String clientId, final String clientSecret)
			throws Exception {
		final String params = "grant_type=client_credentials&scope=http://api.microsofttranslator.com" + "&client_id="
				+ URLEncoder.encode(clientId, ENCODING) + "&client_secret=" + URLEncoder.encode(clientSecret, ENCODING);
