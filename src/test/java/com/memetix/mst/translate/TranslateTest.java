...
public class TranslateTest {
	...
	public void setUp() throws Exception {
		...
		translate = new Translate(clientId, clientSecret);
	...
	public void testTranslate_NoApiKey() throws Exception {
		...
		exception.expectMessage("Server returned HTTP response code: 400 for URL");

		Translate tmp = new Translate("", "");
		assertEquals("Salut", tmp.execute("Hello", Language.ENGLISH, Language.FRENCH));
	}
	...
	public void testTranslate_AutoDetectOrigin() throws Exception {
		assertEquals("Bonjour, mon nom est",
				translate.execute("Hello, my name is", Language.AUTO_DETECT, Language.FRENCH));
	}

	@Test
	public void testTranslate_HTMLContentType() throws Exception {
		Translate.setContentType("text/html");
		assertEquals("<hello>Bonjour, mon nom est</hello>",
				translate.execute("<hello>Hello, my name is</hello>", Language.AUTO_DETECT, Language.FRENCH));
	}
	...
	public void testTranslate_ItalianToGerman_AndBack() throws Exception {
		...
		assertEquals("Hallo welt".toLowerCase(),
				translate.execute("Salve, mondo", Language.ITALIAN, Language.GERMAN).toLowerCase());
	}

	@Test
	public void testTranslate_EnglishToArabic_Unicode() throws Exception {
		// assertEquals("هذا هو بلدي الترجمة للعربية",Translate.execute("This is
		// my translation intended for Arabic", Language.ENGLISH,
		// Language.ARABIC));
	}
	...
	public void testTranslate_SetKeyNoClientIdAndSecret() throws Exception {
		...
		exception.expectMessage(
				"Must provide a Windows Azure Marketplace Client Id and Client Secret - Please see http://msdn.microsoft.com/en-us/library/hh454950.aspx for further documentation");
		String translation = translate.execute("ハローワールド", Language.AUTO_DETECT, Language.ENGLISH);
		assertNotNull(translation);
	}

	/*
	 * @Test public void testTranslate_Exception() throws Exception {
	 * exception.expect(Exception.class);
	 * //exception.expectMessage("INVALID_API_KEY - Please set the API Key with
	 * your Bing Developer's Key"); String result = Translate.execute(
	 * "\"test\" red", Language.AUTO_DETECT, Language.ENGLISH);
	...
	public void testLarge() throws Exception {
		...
		translate.execute(largeText, Language.ENGLISH, Language.FRENCH);
	}
	...
	public void testLargeLimit() throws Exception {
		...
		largeText += " " + largeText;
		largeText += " " + largeText;
		translate.execute(largeText.substring(0, 10240), Language.ENGLISH, Language.FRENCH);
	}
